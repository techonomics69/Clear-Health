				...
				<section class="card" >
					...
					@foreach($category as $key => $data)
					  {{-- <li><a class="btn @if($activeTab == 0) active @elseif($current_tab_id == 'home'.$key) active @endif" data-toggle="tab" href="#home{{$key}}">{{$data}}</a></li> --}}
					   ...
					   <li><a class="btn @if($current_tab_id == 'home'.$key ) active @elseif($activeTab == 0) active @elseif(session('activequiz') == $data) active @endif" data-toggle="tab" href="#home{{$key}}" onclick="setActiveTab()">{{$data}}</a></li>
					 <?php $activeTab++ ?> 
					 @endforeach
					</ul>
