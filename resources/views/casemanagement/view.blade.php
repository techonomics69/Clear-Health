			...
			$.ajax({
				...
				success: function(response){
					...
					if(response.text == null){ 
						$(".chating-section ul").append("<li class='right'>"+"<p id='bottomDivMsg'>"+"<img width='100' src={{URL('/')}}/public/Message_files/" +response.file_name+ ">"+ "<a target='_blank' download='' href={{URL('/')}}/public/Message_files/"+response.file_name+">" + " Download" + "</a>"+"</p>"+"<h5>"+response.msg_date+"<h5>"+"</li>");
					}else{
						$(".chating-section ul").append("<li class='right'>"+"<p id='bottomDivMsg'>"+response.text+"</p>"+"<h5>"+response.msg_date+"<h5>"+"</li>");	
					}
					$("#chating-section").scrollTop($("#chating-section").get(0).scrollHeight);
				}
			});
		});
	});
